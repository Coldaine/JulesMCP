...
**Location:** This playbook lives at `docs/playbooks/organizational/documentation_playbook.md`
...
1. All documentation lives in /docs (see approved exceptions).
2. Use kebab-case filenames: `vosk-model-discovery-flow.md`.
3. Reference crate indexes: place at `docs/reference/crates/<crate>.md` and link to the crate's README:
...
4. Domain troubleshooting (e.g., Vosk model discovery) must live inside the relevant `docs/domains/<domain>/troubleshooting/`.
5. Roadmap lives at `docs/architecture/roadmap.md` and is linked from `docs/architecture.md`.
6. ADRs live under `docs/architecture/adr-XXXX.md` with incrementing numeric IDs and MUST be linked from `docs/architecture.md`.
7. `docs/agents.md` and `CLAUDE.md` MUST both contain the full assistant orientation (overview, workspace map, key commands, feature flags). Keep the contents synchronized, include links to this playbook and docs/standards.md, and retain these files at the documented paths.
